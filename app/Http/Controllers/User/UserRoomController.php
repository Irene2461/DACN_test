...
class UserRoomController extends Controller
{
    public function index(Request $request)
    {
        ...
        $rooms = $rooms->orderByDesc("id")->paginate(3);
    ...
    public function edit($id, Request $request)
    {
        ...
        $cities     = Location::select('id', 'name')->where('type', 1)->get();
        $districts  = Location::select('id', 'name', 'parent_id')->where('type', 2)->get();
        $wards      = Location::select('id', 'name', 'parent_id')->where('type', 3)->get();
        $categories = Category::select('id', 'name')->get();
        $imageList = Image::select('id', 'image')->where('room_id', $id)->get();
        $viewData = [
            'cities'     => $cities,
            'districts'  => $districts,
            'wards'      => $wards,
    ...
    public function update($id, UserRoomRequest $request)
    {
        ...
        // if($request->hasFile("avatar")){
        //     if (File::exists("avatar/".$roomForImg->avatar)) {
        //         File::delete("avatar/".$roomForImg->avatar);
        //     }
        //     $file=$request->file("avatar");
        //     $roomForImg->cover=time()."_".$file->getClientOriginalName();
        //     $file->move(\public_path("/avatar"),$roomForImg->avatar);
        //     $request['avatar']=$roomForImg->avatar;
        // }
    ...
    {   
        $imageList = Image::select('id', 'image')->where('room_id', $id)->where('type',1)->get();
        $videoList = Image::select('id', 'image')->where('room_id', $id)->where('type',2)->get();
        $roomAvatar = Room::find($id)->avatar;
        ...
        return view('user.room.image.edit', $viewData);
    }
